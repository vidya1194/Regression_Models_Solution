...
def main():
    ...
    try:
        ...
        logger.info(f'Linear Regression Train MAE: {evaluate_model(lr_model, x_train, y_train)}')
        logger.info(f'Linear Regression Test MAE: {evaluate_model(lr_model, x_test, y_test)}')
        logger.info("Training Completed for Linear Regression")
        
        dt_model = train_decision_tree(x_train, y_train)        
        logger.info(f'"Decision Tree Train MAE: {evaluate_model(dt_model, x_train, y_train)}')
        logger.info(f'"Decision Tree Test MAE: {evaluate_model(dt_model, x_test, y_test)}')
        logger.info("Training Completed for Decision Tree model")
        
        rf_model = train_random_forest(x_train, y_train)
        logger.info(f'"Random Forest Train MAE: {evaluate_model(rf_model, x_train, y_train)}')
        logger.info(f'"Random Forest Test MAE: {evaluate_model(rf_model, x_test, y_test)}')
